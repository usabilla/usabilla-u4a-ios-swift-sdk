        ...
        describe("FormStoreTests") {
            ...
            context("When calling loadStore", {
                it("Should succeed and return formModel from network if formId isValid", closure: {
                    waitUntil(timeout: 2.0) { done in
                        let promise = FormStore.loadForm(id: "583c0d8ea935028022c145f4", screenshot: nil, customVariables: [:], themeConfig: UsabillaTheme())
                ...
                it("Should succeed and return cached formModel if it's cached", closure: {
                    ...
                    waitUntil(timeout: 2.0) { done in
                        let promise = FormStore.loadForm(id: mockFormModel.appId, screenshot: nil, customVariables: [:], themeConfig: UsabillaTheme())
                ...
                it("Should fail if nothing is retrieved form network nor found in cache", closure: {
                    waitUntil(timeout: 2.0) { done in
                        let promise = FormStore.loadForm(id: "NonExistingFormId", screenshot: nil, customVariables: [:], themeConfig: UsabillaTheme())
                        promise.then { _ in
                            fail()
                        }.catch { _ in
                            done()
            ...
            context("When loading the defaultForm", {
                it("should return a default form successfully", closure: {
                    let defaultFrom = FormStore.loadDefaultForm("", screenshot: nil, customVariables: [:], themeConfig: UsabillaTheme())
