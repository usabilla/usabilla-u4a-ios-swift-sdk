        ...
        describe("NetworkManager") {
            ...
            context("When getFormJson is called") {
                it("should fail getting formModel when formId is wrong") {
                    waitUntil(timeout: 2.0) { done in
                        let promise = NetworkManager.getForm("ThisIsAnInvalidFormId", screenshot: nil, customVariables: nil, themeConfig: UsabillaTheme())
                ...
                it("should retun a formModel when appid is correct") {
                    waitUntil(timeout: 2.0) { done in
                        let promise = NetworkManager.getForm("58bd668e4ee2f5fc304eac59", screenshot: nil, customVariables: nil, themeConfig: UsabillaTheme())
                        promise.then(execute: { _ in
                            done()
                        }).catch(execute: { _ in
                            fail()
