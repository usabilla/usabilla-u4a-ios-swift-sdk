        ...
        describe("CampaignModelTests") {
            ...
            context("When CampaignModel is archived") {
                it("Should have right values when unarchived") {
                    ...
                    let targeting = TargetingOptionsModel(rule: rule, targetingID: "tid", lastModifiedDate: nil)
                    let campaign = CampaignModel(id: campaignID, targeting: targeting, formID: formID, targetingID: targetingID, maximumDisplays: maxDisplays, numberOfTimesTriggered: numberOfTimesTriggered, status: status, createdAt: Date())
