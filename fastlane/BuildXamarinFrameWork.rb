...
private_lane :buildForXamarin do |options|
  ...
  paths = Paths.new(version, project_directory, config)
	...
	sh("xcodebuild -derivedDataPath #{paths.projectDirectory}/build -project #{paths.projectDirectory}/Usabilla.xcodeproj -scheme #{paths.scheme_xamarin_name} -configuration #{config} -sdk iphoneos OTHER_CFLAGS=-fembed-bitcode BITCODE_GENERATION_MODE=bitcode GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=NO BUILD_LIBRARY_FOR_DISTRIBUTION=YES CLANG_ENABLE_CODE_COVERAGE=NO ")
	sh("xcodebuild -derivedDataPath #{paths.projectDirectory}/build -project #{paths.projectDirectory}/Usabilla.xcodeproj -scheme #{paths.scheme_xamarin_name} -configuration #{config}  -sdk iphonesimulator OTHER_CFLAGS=-fembed-bitcode-marker BITCODE_GENERATION_MODE=bitcode GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=NO BUILD_LIBRARY_FOR_DISTRIBUTION=YES CLANG_ENABLE_CODE_COVERAGE=NO ")

	#remove Module name error in swiftinterface files
	sh("find \"#{paths.projectDirectory}/build/Build/Products/#{config}-iphoneos/#{paths.framework_xamarin_name}/Modules/#{paths.scheme_xamarin_name}.swiftmodule/\" -name \"*.swiftinterface\" -exec sed -i -e 's/Usabilla\\.//g' {} \\;") 
	sh("find \"#{paths.projectDirectory}/build/Build/Products/#{config}-iphonesimulator/#{paths.framework_xamarin_name}/Modules/#{paths.scheme_xamarin_name}.swiftmodule/\" -name \"*.swiftinterface\" -exec sed -i -e 's/Usabilla\\.//g' {} \\;") 
...
private_lane :buildForXamarinAndRunUITest do |options|
  ...
  if options[:devices] == nil	
	...
	sh("xcodebuild clean -workspace #{paths.projectDirectory}/Usabilla.xcworkspace -scheme #{paths.scheme_xamarin_name} -configuration #{config} -sdk iphonesimulator")
	sh("xcodebuild build -workspace #{paths.projectDirectory}/Usabilla.xcworkspace -scheme #{paths.scheme_xamarin_name} -configuration #{config} -sdk iphonesimulator")
